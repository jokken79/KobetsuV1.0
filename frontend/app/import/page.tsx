    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {activeTab === 'import' && (
        <div>
          ...
          {/* Server Sync Button for Factories */}
          {importType === 'factories' && (
            <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
              <h3 className="font-semibold text-green-800 mb-2">📁 サーバーから直接インポート</h3>
              <p className="text-sm text-green-700 mb-3">
                E:\factories フォルダのJSONファイルを直接インポートします
              </p>
              <button
                onClick={async () => {
                  try {
                    const response = await fetch('/api/v1/import/factories/folder', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ folder_path: '/network_data', mode: 'sync' })
                    })
                    const result = await response.json()
                    if (result.success) {
                      alert(`インポート完了!\n新規: ${result.imported_count}件\n更新: ${result.updated_count}件`)
                      queryClient.invalidateQueries({ queryKey: ['table-stats'] })
                    } else {
                      alert(`エラー: ${result.message || result.detail}`)
                    }
                  } catch (error) {
                    alert(`エラー: ${error}`)
                  }
